...
def decrypt_n_to_text(n_b64: str) -> str:
    """解密 n（base64）为 UTF-8 文本。"""
    data = base64.b64decode(n_b64)
    if len(data) < 32 or (len(data) - 16) % 16 != 0:
        raise ValueError("加密数据格式无效")
    iv, ciphertext = data[:16], data[16:]
    plaintext = _aes_cbc_decrypt_pkcs7(ciphertext, PSK, iv)
    return plaintext.decode("utf-8")


def decrypt_n_to_json(n_b64: str) -> Dict[str, Any]:
    """解密 n 并解析为 JSON 对象。"""
    text = decrypt_n_to_text(n_b64)
